...
If no jurisdiction is mentioned, return "UNKNOWN".`;
  ...
  const exists = await prisma.jurisdiction.findFirst({
    where: {
      name: {
        equals: name,
        mode: 'insensitive',
  ...
  return exists ? `${exists.name}, ${exists.state}` : null;
}
...
Return up to 3 best matches.`;
  ...
  for (const match of matches) {
    ...
    const matchingRules = rulesets
      ...
      ?.aiRules.filter(r => !match.subcategory || r.subcategory === match.subcategory);
